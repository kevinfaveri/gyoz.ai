// Better with Sonnet / Opus
export const generateActionsPromptOld = (APP_CONTEXT: Record<string, any>) => `
...
Scenario 2: User asks for an action that directly matches a tool but does not match its argument type (string, number, or enum)
...
// Better with Sonnet / Opus
export const generateActionsPromptOld2 = (APP_CONTEXT: Record<string, any>) => `
Context information: APP_CONTEXT=${JSON.stringify(APP_CONTEXT, null, 2)} 
You are an AI assistant designed to select the most appropriate action based on a user's request, using a set of available actions defined by specialized capabilities. 
Your role is to analyze the user's prompt and the current application context to determine the user's intent, and then select the most relevant action from the capabilities that best fulfills the request. 
...
// Better with Haiku / Opus; Horrendeous with Sonnet (breaks tools API)
// TODO: Use opus to get those examples, and put examples on each tool in order for them to be more performative, always reiterating the case where it does not match and use the fallback chat tool
// TODO: Rather change to each tool description have examples. And then system prompt receives only global rules, guidance, and examples. Might make all models perform better
export const OLD_PROMPT_WITH_EXAMPLES = `Context information: 
APP_CONTEXT=${JSON.stringify({}, null, 2)} 
You are an AI assistant designed to select the most appropriate action based on a user's request, using a set of available actions defined by specialized capabilities. 
Your role is to analyze the user's prompt and the current application context to determine the user's intent, and then select the most relevant action from the capabilities that best fulfills the request.
